...
Item {
    ...
    property var thermalCameraViewModel: null
    property ThemeProperties theme: ThemeProperties {}
    property bool framesSwapped: false

    // Calculate maximum size that maintains 16:9 aspect ratio - exact same as original
    property real targetAspect: theme.cameraAspectRatio
    ...
    Image {
        ...
        source: cameraContainer.framesSwapped ?
               (thermalCameraViewModel ? thermalCameraViewModel.currentThermalFrameUrl : "") :
               (cameraViewModel ? cameraViewModel.currentFrameUrl : "")
        cache: false
        smooth: true

        // Constants for source frame dimensions - exact same as original
        readonly property int sourceFrameWidth: theme.sourceFrameWidth
        readonly property int sourceFrameHeight: theme.sourceFrameHeight

        // Calculate the actual rendered image dimensions within the Image component - exact same as original
        ...
        // Function to convert UI coordinates to source frame coordinates - exact same as original
        ...
        // Tracking selection overlay rectangle - exact same as original
        Rectangle {
            id: selectionOverlay
            visible: false
            color: "transparent"
        ...
        Rectangle {
            anchors.fill: parent
            color: "transparent"
            border.color: cameraViewModel ? (cameraViewModel.streaming ? theme.primaryColor : theme.borderColor) : theme.borderColor
            border.width: theme.thickBorderWidth
            radius: theme.buttonRadius
            visible: cameraViewModel ? cameraViewModel.streaming : false
        }

        // Mouse handling for tracking target selection - exact same as original
        MouseArea {
            id: mouseArea
            anchors.fill: parent
            enabled: cameraViewModel ? (cameraViewModel.streaming && cameraViewModel.trackingEnabled) : false
            ...
            onReleased: {
                if (mouse.button === Qt.LeftButton && selectionOverlay.visible) {
                    ...
                    var frameId = cameraViewModel ? cameraViewModel.currentFrameId : 0
        ...
        // Timer to hide selection overlay - exact same as original
        Timer {
    ...
    BusyIndicator {
        anchors.centerIn: parent
        running: cameraViewModel ? (cameraViewModel.cameraStatus.indexOf("Connecting") >= 0) : false
        visible: running
    }
}
